...
one tool call with the model's answers as arguments. The session lives on its own event loop in a
background thread so the controller stays synchronous.
...
@asynccontextmanager
async def _transport(entry: dict):
    """The declared transport, never an inferred one. Yields (read, write)."""
    ...
    if url:
        ...
        async with httpx.AsyncClient(headers=headers, timeout=httpx.Timeout(30, read=300)) as http:
            async with streamable_http_client(url, http_client=http) as (read, write, _get_session_id):
                yield read, write
        return
    from mcp import StdioServerParameters
    from mcp.client.stdio import stdio_client
    params = StdioServerParameters(command=str(entry["command"]), args=[str(a) for a in (entry.get("args") or [])],
...
def _parse(result) -> tuple[dict | None, str, bool]:
    ...
    if data is None and text.strip().startswith("{"):
        try:
            ...
            data = None
    return data, text, bool(getattr(result, "isError", False))


class McpEnvironment(Environment):
    def __init__(self, server: dict | str, observe_tool: str = OBSERVE, reset_tool: str = RESET,
                 timeout: float = 120.0):
        from mcp import ClientSession      # the optional dependency, checked where it is needed
        ...
        self._thread.start()
        self._future = asyncio.run_coroutine_threadsafe(self._serve(ClientSession), self._loop)
    ...
    async def _serve(self, ClientSession) -> None:
        try:
            async with _transport(self.entry) as (read, write):
                async with ClientSession(read, write) as session:
                    init = await session.initialize()
                    self.instructions = str(getattr(init, "instructions", None) or "")
    ...
    def _call(self, name: str, args: dict | None = None):
        ...
        fut = asyncio.run_coroutine_threadsafe(self._session.call_tool(name, args or {}), self._loop)
